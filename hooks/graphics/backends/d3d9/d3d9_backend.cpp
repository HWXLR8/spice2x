...
static bool is_dx9_on_12_enabled() {
    switch (GRAPHICS_9_ON_12_STATE) {
        case DX9ON12_FORCE_OFF:
            log_info("graphics::d3d9", "DirectX 9on12: forced OFF by user (-sp2x-dx9on12)");
            return false;

        case DX9ON12_FORCE_ON:
            log_info("graphics::d3d9", "DirectX 9on12: forced ON by user (-9on12 or -sp2x-dx9on12)");
            return true;
        ...
        default:
            if (GRAPHICS_9_ON_12_REQUESTED_BY_GAME) {
                log_info(
                    "graphics::d3d9",
                    "DirectX 9on12: enabled automatically for current game (tip: use -sp2x-dx9on12 to force on or off)");
                return true;
            } else {
                log_info(
                    "graphics::d3d9",
                    "DirectX 9on12: disabled by default, using DX9 (tip: use -sp2x-dx9on12 to force on or off)");
                return false;
            }
    }
}
        ...
        D3DDISPLAYMODE *pMode)
{
    ...
    if (SUCCEEDED(ret) && pMode) {
        ...
        if (!modified &&
            ...
            log_misc(
                "graphics::d3d9", "swapping width and height for mode {}, {}x{} @ {}Hz (-sp2x-autoorientation)",
                Mode, width, height, refresh);
            pMode->Height = width;
            pMode->Width = height;
            modified = true;
