...
static std::mutex LOW_LATENCY_CLIENT_CREATION_MUTEX;
    ...
    IMMDevice **ppEndpoint)
{
    ...
    // use this opportunity to start the low latency stream (initialized once here, stays around forever)
    if (hooks::audio::LOW_LATENCY_SHARED_WASAPI) {
        std::lock_guard<std::mutex> lock(LOW_LATENCY_CLIENT_CREATION_MUTEX);
        if (hooks::audio::LOW_LATENCY_CLIENT == nullptr) {
            hooks::audio::LOW_LATENCY_CLIENT = hooks::audio::LowLatencyAudioClient::Create(*ppEndpoint);
        }
    }
