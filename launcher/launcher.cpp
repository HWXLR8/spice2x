...
int main_implementation(int argc, char *argv[]) {
    ...
    if (options[launcher::Options::spice2x_IIDXWindowedTDJ].value_bool()) {
        games::iidx::TDJ_MODE = true;
        GRAPHICS_WINDOWED = true;
        games::iidx::SUBSCREEN_OVERLAY_SIZE = "fullscreen";
        // yes, 4, because it results in a tiny secondary window, which is what we want to
        // discourage users from clicking on it & use the subscreen overlay instead
        games::iidx::SCREEN_MODE = "4";
    }
